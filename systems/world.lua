				...
				for x=sx, ex do
					...
					for i=1, 2 do
						...
						if holdTile then
							...
							local dtt
							local tileSheet = holdTile.tileSheet
							if tileSheet and tileSheet.x and tileSheet.y and (tileSheet.x > 1 or tileSheet.y > 1) and holdTile.speed and holdTile.playback then
								local sheetX, sheetY = tileSheet.x, tileSheet.y
								...
								if pb ~= "stop" then
									if holdTile.lastPlayback ~= pb then
										...
										if pb == "reversestart" then ct = (sheetX*sheetY-1)*time end
									end
									if pb == "reverseplay" or pb == "reversestart" or pb == "reverseloop" then
										if ct < 0 then
											...
											while ct < 0 do
												local sub = (sheetX*sheetY)*time
												ct = ct + sub; holdTile.curTime = holdTile.curTime + sub
											end
										end
									else
										if ct > (sheetX*sheetY)*time then
											if pb == "play" or pb == "start" then
												holdTile.playback = "stop"
												ct = (sheetX*sheetY-1)*time
											end
											while ct > (sheetX*sheetY)*time do
												local sub = (sheetX*sheetY)*time
												ct = ct - sub; holdTile.curTime = holdTile.curTime - sub
											end
										end
									...
									local hct = ct
									ty = math.ceil((math.floor(hct/time)+1)/sheetX)
									tx = (math.floor(hct/time)+1)-((ty-1)*sheetX)
									tileSheet.tx, tileSheet.ty = tx, ty
					...
					for x=sx, ex do
						...
						if map.tiles.floor[y] and map.tiles.floor[y][x] then
							...
							if holdTile.drawable then
								local drawQuad
								local tileSheet = holdTile.tileSheet
								if tileSheet and tileSheet.tiles and tileSheet.tiles and tileSheet.tiles[tileSheet.ty] and tileSheet.tiles[tileSheet.ty][tileSheet.tx] then drawQuad = tileSheet.tiles[tileSheet.ty][tileSheet.tx] end
								if pri == world.runPriority[1] then
									if drawQuad then
										ui.draw({image.getImage(holdTile.imageKey), drawQuad}, (x-1)*ts, (y-1)*ts, ts*tileSheet.x, ts*tileSheet.y)
									else
